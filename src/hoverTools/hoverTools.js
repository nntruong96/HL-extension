...
$.get(chrome.runtime.getURL('src/hoverTools/hoverTools.html'), (data) => {
  ...
  inputTag = hoverToolEl.find('.highlighter-input-tag')[0];
  ...
  //   inputTag.addEventListener('keyup', onChangeInputTag);
});
...
function moveToolbarToHighlight(highlightEl, cursorX) {
  ...
  if (cursorX !== undefined) {
    ...
    const index = currentHighlightEl.getAttribute('data-highlight-id');
    let url = window.location.hostname + window.location.pathname;
    getHighlight(url, index, (data) => {
...
function hide() {
  console.log('hide', highlightClicked, 'hoverToolTimeout', hoverToolTimeout);
  if (currentHighlightEl && (highlightClicked || hoverToolTimeout)) {
    const index = currentHighlightEl.getAttribute('data-highlight-id');
    let url = window.location.hostname + window.location.pathname;
    getHighlight(url, index, (data) => {
      let tag = inputTag.value;
      console.log('tag', tag, 'data.tag', data.tag);
      if (data && tag !== data.tag) {
        console.log('update tag');
        return updateTag(url, index, tag, () => {
          inputTag.value = '';
        });
      }
      tag = '';
...
function onChangeColorBtnClicked() {
  console.log('onChangeColorBtnClicked');
  const highlightId = currentHighlightEl.getAttribute('data-highlight-id');
  const highlights = $(
    `.highlighter--highlighted[data-highlight-id='${highlightId}']`
  );
